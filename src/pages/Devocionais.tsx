...
import { ExternalLink, Sparkles, Share2, BookOpen, ChevronRight, ScrollText, Layout, Quote } from "lucide-react";
...
export default function Devocionais() {
    ...
    const infoLivros: { [key: string]: { nome: string } } = {
        ...
        // ...adicione os demais conforme necessário para cobrir os 66 livros
    };
    ...
    const selectedData = publicacoes.find(p => p.id === selectedId) || publicacoes[0];

    // Link EXATO para o versículo na Life.Church (NAA - ID 1840)
    // Formato: LIVRO.CAPITULO.VERSICULO (Ex: JHN.21.25)
    const linkLifeChurch = (item: any) => item 
        ? `https://www.bible.com/pt/bible/1840/${item.livroRef}.${item.capituloRef}.${item.versiculoRef || '1'}`
        : "#";

    return (
        <div className="min-h-screen bg-slate-50 pt-28 pb-10 px-4 md:px-8 font-body">
            <div className="max-w-[1800px] mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                            ...
                            publicacoes.map((e) => (
                                <div key={e.id} className="space-y-4">
                                    <article 
                                        ...
                                        className={`bg-white rounded-[2.5rem] border cursor-pointer overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col md:flex-row ${selectedId === e.id ? 'ring-4 ring-emerald-500/10 border-emerald-500' : 'border-slate-100'}`}
                                    >
                                    ...
                                    {/* MOBILE: REFERÊNCIA COLADA (SEM TEXTO BÍBLICO, SÓ REFERÊNCIA) */}
                                    {selectedId === e.id && (
                    ...
                    {/* DIREITA: DESKTOP (TEXTO MANUAL + DESIGN DISCRETO) */}
                            ...
                            <div className="bg-white rounded-[3rem] border border-slate-200 shadow-2xl overflow-hidden flex flex-col min-h-[500px] relative">
                                ...
                                <div className="p-10 flex flex-col items-center justify-center flex-1 text-center space-y-6">
                                    <div className="flex items-center gap-2 px-4 py-1.5 bg-slate-50 rounded-full border border-slate-100 mb-2">
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 italic">Base Bíblica</span>
                                    </div>

                                    {/* Título Reduzido e Legível */}
                                    <div className="space-y-1">
                                        <h4 className="text-4xl font-display font-black text-slate-900 uppercase italic leading-none tracking-tighter">
                                            {selectedData ? infoLivros[selectedData.livroRef]?.nome || selectedData.livroRef : "---"}
                                        </h4>
                                    ...
                                    {/* TEXTO BÍBLICO MANUAL (Pequeno e Legível) */}
