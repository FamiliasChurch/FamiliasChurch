...
export default function MembershipCard({ user }: { user: UserData }) {
  ...
  useEffect(() => {
      const checkStatus = async () => {
          ...
          try {
            ...
            console.log("Aguardando permissão...", error);
          }
      };
      checkStatus();
  }, [user.email]);
  ...
  return (
    <div className="flex flex-col items-center gap-8 w-full max-w-md mx-auto font-sans animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* --- O CARTÃO PREMIUM (CORRIGIDO) --- */}
      {/* ALTERAÇÃO: Removido 'aspect' fixo. Agora é h-auto com min-h para garantir que caiba tudo. */}
      <div className={`w-full h-auto min-h-[260px] rounded-[2rem] p-6 shadow-2xl relative overflow-hidden transition-transform hover:scale-[1.02] duration-500 group ${theme.bg}`}>
        ...
        <div className="relative z-10 flex flex-col h-full gap-5"> {/* gap-5 para espaçar bem */}
          
          {/* TOPO: LOGO E TÍTULO */}
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-3">
              <div className="p-0.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-lg shrink-0">
                 <img src="/logo.webp" alt="Logo" className="w-10 h-10 rounded-full object-cover" />
              </div>
              <div className="flex flex-col min-w-0">
                <h3 className={`font-serif font-black tracking-wide text-lg leading-none ${theme.text} drop-shadow-sm truncate`}>
                  FAMÍLIAS CHURCH
                </h3>
            ...
            <div className={`opacity-60 ${theme.text} shrink-0`}>
          ...
          <div className="flex items-end gap-4">
            {/* Foto */}
            <div className={`w-20 h-24 sm:w-24 sm:h-28 rounded-xl p-1 bg-white/10 backdrop-blur-sm border ${theme.accent} shadow-2xl relative shrink-0`}>
                {user.foto ? (
            ...
            <div className="flex-1 space-y-2 pb-1 min-w-0">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <p className={`font-black uppercase text-xs tracking-wide ${theme.text} truncate`}>{user.cargo}</p>
                </div>
                <div>
          ...
          {/* RODAPÉ: ID E QR */}
          <div className={`flex justify-between items-end pt-3 border-t ${theme.accent} mt-auto`}>
             <div className="min-w-0 flex-1 mr-2">
                <p className={`text-[6px] uppercase font-black tracking-[0.3em] opacity-50 mb-0.5 ${theme.text}`}>Registro Administrativo</p>
                <p className={`font-mono text-sm font-black tracking-widest drop-shadow-md ${theme.text} truncate`}>
             ...
             <div className="relative group/qr shrink-0">
                <div className="bg-white p-1 rounded-lg shadow-lg">
                    {statusValidacao === "Aprovado" && user.ra ? (
                        ...
                        <div className="w-[40px] h-[40px] bg-slate-100 rounded flex flex-col items-center justify-center text-center p-0.5 border border-slate-200">
